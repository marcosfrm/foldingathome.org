...
const sidebarGroups = groupsFromOverride(SECTION_OVERRIDES.alpha!)
const latestGuide = { slug: 'guides/v8-5', label: 'v8.5 Client Users Guide' }

useHead({ title: 'Alpha — Folding@home' })
</script>

<template lang="pug">
.layout.has-sidebar
  SectionSidebar(:groups="sidebarGroups" current-path="alpha")
  article.page
    header.page-head
      h1.section-title Alpha
    .body
      p
        | Here you will find the latest #[b alpha] release of the Folding@home
      ...
      p
        ...
        | the latest public release, visit
        | #[NuxtLink(to="/download") Start folding].

      h3 Release packages
      ClientOnly
        DownloadPicker(release="alpha" mode="release")
        template(#fallback)
          p.fah-fallback Loading downloads…
      h3 Debug packages
      ClientOnly
        DownloadPicker(release="alpha" mode="debug")
        template(#fallback)
          p.fah-fallback Loading downloads…
      .previous-releases
